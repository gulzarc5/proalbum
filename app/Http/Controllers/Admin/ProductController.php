...
class ProductController extends Controller
{
    ...
    public function productOptionDetailAdd(Request $request)
    {
        $request->validate([
            'name' => 'required',
    ...
    public function productOptionEditForm($product_id,$tab = null)
    {
        try {
            $product_id = decrypt($product_id);
        }catch(DecryptException $e) {
        ...
        // dd($option);
        return view('admin.products.edit_product_option',compact('option','size','product','tab'));
    }

    public function productOptionAdd(Request $request)
    {
        $request->validate([
            'p_id' => 'required',    
            'option' => 'required',   
        ...
        ////////////Product Options///////////
        $option_sort = DB::table('product_option')->where('p_id',$p_id)->orderBy('sort','desc')->first();
        if (isset($option) && !empty($option)) {
            $sort_option = 1;
            if (isset($option_sort->sort) && !empty($option_sort->sort)) {
                $sort_option = $option_sort->sort+1;
            }
            for ($i=0; $i < count($option); $i++) { 
                if (isset($option[$i]) && isset($option[$i])) {
                DB::table('product_option')->insert([
        ...
        return redirect()->route('admin.product_option_edit_form',['p_id'=>encrypt($p_id)]);

    }

    public function productSizeEditForm($id)
    {
        try {
            $id = decrypt($id);
        }catch(DecryptException $e) {
        ...
        $size = DB::table('product_size')->where('p_id',$id)->get();
        return view('admin.products.edit_sizes',compact('size','id'));
    }

    public function productNewSize(Request $request)
    {
        $request->validate([
            'p_id' => 'required',    
            'swidth.*' => 'required',
        ...
        if (isset($swidth) && !empty($swidth)) {
            $sort_size = 1;
            $size_sort = DB::table('product_size')->where('p_id',$p_id)->orderBy('sort','desc')->first();
            if ($size_sort) {
                if (isset($size_sort->sort) && !empty($size_sort->sort)) {
                    $sort_size = $size_sort->sort+1;
                }
            }
            for ($i=0; $i < count($swidth); $i++) { 
               if (isset($swidth[$i]) && isset($sheight[$i]) && isset($displaysize[$i]) && !empty($swidth[$i]) && !empty($sheight[$i]) && !empty($displaysize[$i]) && isset($sprice[$i]) && !empty($sprice[$i])) {
    ...
    public function productSizeUpdate(Request $request)
    {
        $request->validate([
            ...
            'sHeight' => 'required',
            'sWidth' => 'required',
            'sDisplayName' => 'required',
            's_price' => 'required',
        ]);
        DB::table('product_size')
        ->where('id',$request->input('size_id'))
        ->update([
            'height' => $request->input('sHeight'),
            'width' => $request->input('sWidth'),
            'display_name' => $request->input('sDisplayName'),
            'extra_page_price' => $request->input('s_price'),
            'updated_at' => Carbon::now()->setTimezone('Asia/Kolkata')->toDateTimeString(),
        ]);
        return redirect()->back();
    }
    ...
    public function productImageEdit($product_id)
    {
        ...
        $main_image = DB::table('products')->where('id',$product_id)->first();
        $all_image = DB::table('product_images')->where('product_id',$product_id)->get();
        return view('admin.products.product_image',compact('main_image','all_image'));
    }

    public function productImageCoverSet($p_id,$image_id)
    {
        try {
            $p_id = decrypt($p_id);
        ...
        $image = DB::table('product_images')->where('id',$image_id)->first();
        if ($image) {
            DB::table('products')
                ->where('id',$p_id)
                ->update([
